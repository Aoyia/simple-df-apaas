...
/**
 * 请求插件
 * @typedef {Object} RequestPlugin
 * @property {Function} install - 安装插件的方法
 * @param {Vue} Vue - Vue实例
  ...
  install(Vue, opts) {
        ...
        if (err.response) {
          if (HTTP_ERROR_CONSTANT[err.response.status]) {
            Vue.prototype.$message.error(
              HTTP_ERROR_CONSTANT[err.response.status].errorMsg
            );
          } else if (
            err.response.status >=
    ...
    Vue.removeInterceptorsRequest = function (interceptorKey) {
      if (requestInterceptorMap.get(interceptorKey)) {
        instance.interceptors.request.eject(
          requestInterceptorMap.get(interceptorKey)
        );
      }
    };
    ...
    Vue.removeInterceptorsResponse = function (interceptorKey) {
      if (responseInterceptorMap.get(interceptorKey)) {
        instance.interceptors.request.eject(
          responseInterceptorMap.get(interceptorKey)
        );
      }
    };
    ...
    Vue.request = function (urlConfig, needShowMessage) {
      ...
      if (urlConfig.url.indexOf("?") !== -1) {
        urlConfig.url =
          urlConfig.url + "&" + "timestamp=" + new Date().valueOf();
      } else {
        urlConfig.url =
          urlConfig.url + "?" + "timestamp=" + new Date().valueOf();
      }
